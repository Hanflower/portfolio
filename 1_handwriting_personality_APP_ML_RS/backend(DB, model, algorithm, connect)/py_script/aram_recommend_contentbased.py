...
def contentbased_recommend(userID, recent_contentID, interest_typeList) : # recent_contentID는 정수, interest_typeList는 정수가 있는 리스트
    ...
    sql = "SELECT userID, userName, postText, writingImage, postDate, contentTitle, contentWriter FROM USER_POST INNER JOIN CONTENTS ON USER_POST.contentID = CONTENTS.contentID ORDER BY postDate DESC"
    cur.execute(sql)
    rows = cur.fetchall()

    # 데이터프레임으로 변환
    post_df = pd.DataFrame(rows, columns=['userID', 'userName', 'postText', 'writingImage', 'postDate', 'contentTitle', 'contentWriter'])
    con.close()
